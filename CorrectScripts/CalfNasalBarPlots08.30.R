...
top10Phylum <- phylum[c(1:10),] #to select the top 10 most abundant phylum
sum(top10Phylum$Ave_Abundance)### the total of the community composed of the top 10
top10Phylum$Phylum <- as.factor(top10Phylum$Phylum)
...
a <- ggplot(top10Phylum, aes(x = "", y = Ave_Abundance, fill = fct_infreq(Phylum))) +
  geom_bar(width = 2, stat = "identity") +
  coord_polar(theta = "y", start = 0.0) +
  theme_minimal() +
  scale_fill_manual(values = my_colors) +
  guides(fill = guide_legend(title = "Top 10 Phylum", keywidth = 1.5, keyheight = 1, ncol = 1)) +
...
top10P <- merge(meta_otu_tax, top10Phylum, by.x = "Phylum", by.y="Phylum")
top10P$Phylum <- as.factor(top10P$Phylum)
levels(top10P$Phylum)
str(top10P)
...
str(PhylumAB)
sum(PhylumAB$taxa.average)
PhylumAB$Phylum <- factor(PhylumAB$Phylum)
#write.csv(PhylumAB,"phylumHealthyNasalHealthy.csv")

#Plot the graph 
x <- ggplot(PhylumAB, aes(x = d, y = taxa.average, fill =Phylum)) + 
  ...
  ylab(paste0("Relative Abundance (Top 10)")) +  labs(x='Day')


### Abundance at a family level
family <- meta_otu_tax %>% 
  group_by(Family) %>% 
  summarise(Ave_Abundance = (sum(Abundance)/51)) ## the total number of samples (51)
attach(family)
sum(family$Ave_Abundance)
family <- family[order(-Ave_Abundance),]
#write.csv(family,"familyHealthyNasal.csv")

top10family <- family[c(1:10),] #to select the top 10 most abundant phylum
sum(top10family$Ave_Abundance) ### the total of the community composed of the top 10

my_colorsB <- c("orange","#DA5734", "#508578", "#CD9BCD",
  "#AD6F3B", "#673770","#D14385", "#653936", "#C84348", 
  "#8569D5", "#5E738F","#D1A33D", "#8A7C64", "#599861", "black"
)

b <- ggplot(top10family, aes(x = "", y = Ave_Abundance, fill = fct_infreq(Family))) +
  geom_bar(width = 2, stat = "identity") +
  coord_polar(theta = "y", start = 0.0) +
  theme_minimal() +
  scale_fill_manual(values = my_colorsB) +
  guides(fill = guide_legend(title = "Top 10 Family", keywidth = 1.5, keyheight = 1, ncol = 1)) +
  theme(legend.text=element_text(size=8)) +
  theme(legend.text = element_text(size=11, face = "italic")) +
  theme(strip.text = element_text(size = 13, face = "bold")) +
  theme(legend.text = element_text(size=12)) +
...
top10F <- merge(meta_otu_tax, top10family, by.x = "Family", by.y="Family")
top10F$Family <- as.factor(top10F$Family)
levels(top10F$Family)
str(top10F)

FamilyAB <- top10F %>% 
  group_by(Row.names, d, Family) %>% 
  summarise(taxa.sum = sum(Abundance)) %>%
  group_by(d, Family) %>%
  summarise(taxa.average = mean(taxa.sum)) ### relative abundance, doesn't ass 
str(FamilyAB)
sum(FamilyAB$taxa.average)
FamilyAB$Family <- factor(FamilyAB$Family)
#write.csv(FamilyAB,"familyHealthyNasalTop10.csv")

my_colorsD <- c(
  '#1f78b4','#b3df8a',"#653936","#5E738F",'#33a03c','#fb9a99',"#AD6F3B",'#e31a1c',
  '#fdbf6f','#ff7f00','#cab3d6','#6a3d9a','#ffff99','#b15938', 
  "#CBD588", "#5F7FC7", "orange","#DA5734", "#508578", "#CD9BCD",
  "#AD6F3B", "#673770","#D14385", "#653936", "#C84348", 
  "#8569D5", "#5E738F","#D1A33D", "#8A7C64", "#599861", "black"
...
y <- ggplot(FamilyAB, aes(x = d, y = taxa.average, fill =Family)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
  scale_fill_manual(values = my_colors) +
  ylim(c(0,1)) +
...
#write.csv(genus,"genusHealthyNasal.csv")

top10genus <- genus[c(1:10),] #to select the top 10 most abundant phylum
...
c <- ggplot(top10genus, aes(x = "", y = Ave_Abundance, fill = fct_infreq(Genus))) +
  geom_bar(width = 2, stat = "identity") +
  coord_polar(theta = "y", start = 0.0) +
  theme_minimal() +
  scale_fill_manual(values = my_colorsHG) +
  guides(fill = guide_legend(title = "Top 10 Genus", keywidth = 1.5, keyheight = 1, ncol = 1)) +
  theme(legend.text=element_text(size=8)) +
  theme(legend.text = element_text(size=11, face = "italic")) +
  theme(strip.text = element_text(size = 13, face = "bold")) +
  theme(legend.text = element_text(size=12)) +
...
top10G <- merge(meta_otu_tax, top10genus, by.x = "Genus", by.y="Genus")
top10G$Genus <- as.factor(top10G$Genus)
levels(top10G$Genus)
str(top10G)

genusAB <- top10G %>% 
  group_by(Row.names, d, Genus) %>% 
  summarise(taxa.sum = sum(Abundance)) %>%
  group_by(d, Genus) %>%
  summarise(taxa.average = mean(taxa.sum)) ### relative abundance, doesn't ass 
str(genusAB)
...
#write.csv(genusAB,"genusHealthyNasalTop10.csv")

#Plot the graph 
z <- ggplot(genusAB, aes(x = d, y = taxa.average, fill =Genus)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
  scale_fill_manual(values = my_colors) +
  ylim(c(0,1)) +
...
ggarrange(u,x,v,y,w,z, labels = c("a", "b", "c", "d", "e", "f"),
          nrow=3, ncol = 2)

#### checking Specie level
species <- meta_otu_tax %>% 
  group_by(Species) %>% 
  summarise(Ave_Abundance = (sum(Abundance)/51)) ## the total number of samples (51)
attach(species)
sum(species$Ave_Abundance)
species <- species[order(-Ave_Abundance),]
write.csv(species,"speciesHealthyNasal.csv")

top10species <- species[c(1:10),] #to select the top 10 most abundant phylum
sum(top10species$Ave_Abundance) #the total of the community composed of the top 10
...
top10S <- merge(meta_otu_tax, top10species, by.x = "Species", by.y="Species")
top10S$Species <- as.factor(top10S$Species)
levels(top10S$Species)
str(top10S)
...
speciesAB <- top10S %>% 
  ...
  summarise(taxa.average = mean(taxa.sum)) ### relative abundance, doesn't ass 
str(speciesAB)
sum(speciesAB$taxa.average)
speciesAB$Species <- factor(speciesAB$Species)
write.csv(speciesAB,"speciesHealthyNasalTop10.csv")

#Plot the graph 
f <- ggplot(speciesAB, aes(x = d, y = taxa.average, fill =Species)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
  scale_fill_manual(values = my_colors) +
  ylim(c(0,1)) +
...
mycoAD <- mycoD %>% 
  ...
  summarise(taxa.average = mean(taxa.sum)) ### relative abundance, doesn't ass 
str(mycoAD)
sum(mycoAD$taxa.average) #0.5118467
mycoAD$Species <- factor(mycoAD$Species)
write.csv(mycoAD,"MycoplasmaDraxxinNasalTreatment.csv")

g <- ggplot(mycoAD, aes(x = d, y = taxa.average, fill =Species)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
  scale_fill_manual(values = my_colors) +
  ylim(c(0,1)) +
...
##check how mycoplasma species are different across time points 
lactoAD <- lactoD %>% 
  group_by(Row.names, treatment, d, Species) %>% 
  summarise(taxa.sum = sum(Abundance)) %>%
  group_by(treatment, d, Species) %>%
  summarise(taxa.average = mean(taxa.sum)) ### relative abundance, doesn't ass 
str(lactoAD)
sum(lactoAD$taxa.average) #1.066677
lactoAD$Species <- factor(lactoAD$Species)
write.csv(lactoAD,"LactoDraxxinNasalTreatment.csv")

ggplot(lactoAD, aes(x = d, y = taxa.average, fill =Species)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
  scale_fill_manual(values = my_colors) +
  facet_grid(treatment~.)+
...
top10phylumD <- phylumD[c(1:10),] #to select the top 10 most abundant phylum
sum(top10phylumD$Ave_Abundance) #the total of the community composed of the top 10
top10PD <- merge(meta_otu_taxD, top10phylumD, by.x = "Phylum", by.y="Phylum")
top10PD$Phylum <- as.factor(top10PD$Phylum)
levels(top10PD$Phylum)
str(top10PD)
...
phylumAD <- top10PD %>% 
  ...
  summarise(taxa.average = mean(taxa.sum)) ### relative abundance, doesn't ass 
str(phylumAD)
sum(phylumAD$taxa.average)
phylumAD$Phylum <- factor(phylumAD$Phylum)
write.csv(phylumAD,"phylumDraxxinNasalTop10.csv")
...
a
u <- ggplot(phylumAD, aes(x = d, y = taxa.average, fill =Phylum)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
...
#### ------------------------Draxxin family
familyD <- meta_otu_taxD %>% 
  group_by(Family) %>% 
  summarise(Ave_Abundance = (sum(Abundance)/22)*100) ## the total number of samples (51)
attach(familyD)
sum(familyD$Ave_Abundance)
familyD <- familyD[order(-Ave_Abundance),]
write.csv(familyD,"familyDraxxinNasal.csv")

top10familyD <- familyD[c(1:10),] #to select the top 10 most abundant phylum
sum(top10familyD$Ave_Abundance) #the total of the community composed of the top 10
top10FD <- merge(meta_otu_taxD, top10familyD, by.x = "Family", by.y="Family")
top10FD$Family <- as.factor(top10FD$Family)
levels(top10FD$Family)
str(top10FD)
...
familyAD <- top10FD %>% 
  ...
  summarise(taxa.average = mean(taxa.sum)) ### relative abundance, doesn't ass 
str(familyAD)
sum(familyAD$taxa.average)
familyAD$Family <- factor(familyAD$Family)
write.csv(familyAD,"familyDraxxinNasalTop10.csv")

#Plot the graph 
my_colorsD1 <- c(
  '#1f78b4',"#8A7C64",'#b3df8a','#33a03c','#fb9a99','#e31a1c',
  '#ffff99','#fdbf6f','#ff7f00','#cab3d6','#6a3d9a','#ffff99','#b15938', 
  "#CBD588", "#5F7FC7", "orange","#DA5734", "#508578", "#CD9BCD",
  "#AD6F3B", "#673770","#D14385", "#653936", "#C84348", 
  "#8569D5", "#5E738F","#D1A33D", "#8A7C64", "#599861", "black"
)

v <- ggplot(familyAD, aes(x = d, y = taxa.average, fill =Family)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
  scale_fill_manual(values = my_colorsD1) +
  ylim(c(0,1)) +
  guides(fill = guide_legend(reverse = F, keywidth = 1.5, keyheight = .6, ncol = 1)) +
  guides(fill = guide_legend(reverse = F, keywidth = 1.5, keyheight = .6, ncol = 1)) +
  theme(legend.text=element_text(size=8, face="italic")) +
  ...
  theme(axis.title.x = element_text(color="black", size=11, face="bold"), axis.title.y = element_text(color="black", size=11, face="bold")) + 
  theme(axis.text.x = element_text(color = "black", size = 11), axis.text.y = element_text(color = "black", size = 11)) +
  #heme(text=element_text(family="Times New Roman")) +
  ylab(paste0("Relative Abundance (Top 10)")) +  labs(x='Day')

#### ------------------------Draxxin genus
genusD <- meta_otu_taxD %>% 
  group_by(Genus) %>% 
  summarise(Ave_Abundance = (sum(Abundance)/22)*100) ## the total number of samples (51)
attach(genusD)
sum(genusD$Ave_Abundance)
genusD <- genusD[order(-Ave_Abundance),]
write.csv(genusD,"genusDraxxinNasal.csv")

top10genusD <- genusD[c(1:10),] #to select the top 10 most abundant phylum
sum(top10genusD$Ave_Abundance) #the total of the community composed of the top 10
top10GD <- merge(meta_otu_taxD, top10genusD, by.x = "Genus", by.y="Genus")
top10GD$Genus <- as.factor(top10GD$Genus)
levels(top10GD$Genus)
str(top10GD)
...
genusAD <- top10GD %>% 
  ...
  summarise(taxa.average = mean(taxa.sum)) ### relative abundance, doesn't ass 
str(genusAD)
sum(genusAD$taxa.average)
genusAD$Genus <- factor(genusAD$Genus)
write.csv(genusAD,"genusDraxxinNasalTop10.csv")
...
my_colorsD2 <- c(
  '#a6cee3','#b3df8a','#33a03c','#fb9a99','#e31a1c',
  '#fdbf6f','#ff7f00','#ffff99','#cab3d6','#6a3d9a','#ffff99','#b15938', 
  "#CBD588", "#5F7FC7", "orange","#DA5734", "#508578", "#CD9BCD",
  "#AD6F3B", "#673770","#D14385", "#653936", "#C84348", 
  "#8569D5", "#5E738F","#D1A33D", "black"
)
w <- ggplot(genusAD, aes(x = d, y = taxa.average, fill =Genus)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
...
mycoAN <- mycoN %>% 
  ...
  summarise(taxa.average = mean(taxa.sum)) ### relative abundance, doesn't ass 
str(mycoAN)
sum(mycoAN$taxa.average) #0.1075119
mycoAN$Species <- factor(mycoAN$Species)
write.csv(mycoAN,"MycoplasmaNuflurNasalTreatment.csv")

ggplot(mycoAN, aes(x = d, y = taxa.average, fill =Species)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
  scale_fill_manual(values = my_colors) +
  facet_grid(treatment~.)+
...
lactoAN <- lactoN %>% 
  ...
  summarise(taxa.average = mean(taxa.sum)) ### relative abundance, doesn't ass 
str(lactoAN)
sum(lactoAN$taxa.average) #1.114489
lactoAN$Species <- factor(lactoAN$Species)
write.csv(lactoAN,"LactoNuflurNasalTreatment.csv")

ggplot(lactoAN, aes(x = d, y = taxa.average, fill =Species)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
  scale_fill_manual(values = my_colors) +
  facet_grid(treatment~.)+
...
#Plot the graph 
my_colorsN <- c(
  '#1f78b4','#b3df8a',"#653936","#5E738F",'#fb9a99','#e31a1c',
  '#fdbf6f','#ff7f00','#cab3d6','#6a3d9a','#ffff99','#b15938', 
  "#CBD588", "#5F7FC7", "orange","#DA5734", "#508578", "#CD9BCD",
  "#AD6F3B", "#673770","#D14385", "#653936", "#C84348", 
  "#8569D5", "#5E738F","#D1A33D", "#8A7C64", "#599861", "black"
...
a
x <- ggplot(phylumAN, aes(x = d, y = taxa.average, fill =Phylum)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
  scale_fill_manual(values = my_colorsN) +
  labs(title = "Nuflur") +
  ylim(c(0,1)) +
  guides(fill = guide_legend(reverse = F, keywidth = 1.5, keyheight = .6, ncol = 1)) +
  guides(fill = guide_legend(reverse = F, keywidth = 1.5, keyheight = .6, ncol = 1)) +
...
#### ------------------------Nuflur family
familyN <- meta_otu_taxN %>% 
  group_by(Family) %>% 
  summarise(Ave_Abundance = (sum(Abundance)/11)*100) ## the total number of samples (51)
attach(familyN)
sum(familyN$Ave_Abundance)
familyN <- familyN[order(-Ave_Abundance),]
write.csv(familyN ,"familyNuflurNasal.csv")

top10familyN <- familyN[c(1:10),] #to select the top 10 most abundant phylum
sum(top10familyN$Ave_Abundance) #the total of the community composed of the top 10
top10FN <- merge(meta_otu_taxN, top10familyN, by.x = "Family", by.y="Family")
top10FN$Family <- as.factor(top10FN$Family)
levels(top10FN$Family)
str(top10FN)
...
familyAN <- top10FN %>% 
  ...
  summarise(taxa.average = mean(taxa.sum)) ### relative abundance, doesn't ass 
str(familyAN)
sum(familyAN$taxa.average)
familyAN$Family <- factor(familyAN$Family)
write.csv(familyAN,"familyNuflurNasalTop10.csv")

#Plot the graph 
my_colorsN1 <- c(
  '#a6cee3','#1f78b4',"#8A7C64",'#b3df8a','#33a03c','#fb9a99',
  '#fdbf6f',"#C84348",'#ff7f00','#cab3d6','#6a3d9a','#ffff99','#b15938', 
  "#CBD588", "#5F7FC7", "orange","#DA5734", "#508578", "#CD9BCD",
  "#AD6F3B", "#673770","#D14385", "#653936", "#C84348", 
  "#8569D5", "#5E738F","#D1A33D", "#8A7C64", "#599861", "black"
)
...
y <- ggplot(familyAN, aes(x = d, y = taxa.average, fill =Family)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
  scale_fill_manual(values = my_colorsN1) +
  ylim(c(0,1)) +
  guides(fill = guide_legend(reverse = F, keywidth = 1.5, keyheight = .6, ncol = 1)) +
  guides(fill = guide_legend(reverse = F, keywidth = 1.5, keyheight = .6, ncol = 1)) +
  theme(legend.text=element_text(size=8, face="italic")) +
...
#### ------------------------Nuflur genus
genusN <- meta_otu_taxN %>% 
  group_by(Genus) %>% 
  summarise(Ave_Abundance = (sum(Abundance)/11)*100) ## the total number of samples (51)
attach(genusN)
sum(genusN$Ave_Abundance)
genusN <- genusN[order(-Ave_Abundance),]
write.csv(genusN,"genusNNuflurNasal.csv")

top10genusN <- genusN[c(1:10),] #to select the top 10 most abundant phylum
sum(top10genusN$Ave_Abundance) #the total of the community composed of the top 10
top10GN <- merge(meta_otu_taxN, top10genusN, by.x = "Genus", by.y="Genus")
top10GN$Genus <- as.factor(top10GN$Genus)
levels(top10GN$Genus)
str(top10GN)
...
str(genusAN)
sum(genusAN$taxa.average)
genusAN$Genus <- factor(genusAN$Genus)
write.csv(genusAN,"genusNuflurNasalTop10.csv")
...
my_colorsD2 <- c(
  '#1f78b4','#b3df8a','#33a03c','#fb9a99','#e31a1c',
  '#fdbf6f','#ff7f00','#ffff99','#cab3d6','#6a3d9a','#ffff99','#b15938', 
  "#CBD588", "#5F7FC7", "orange","#DA5734", "#508578", "#CD9BCD",
  "#AD6F3B", "#673770","#D14385", "#653936", "#C84348", 
  "#8569D5", "#5E738F","#D1A33D", "black"
...
my_colorsN2 <- c(
  '#a6cee3','#1f78b4','#b3df8a','#33a03c','#fb9a99','#e31a1c',
  "#8569D5",'#ff7f00','#cab3d6', '#b15938','#6a3d9a','#ffff99','#b15938', 
  "#CBD588", "#5F7FC7", "orange","#DA5734", "#508578", "#CD9BCD",
  "#AD6F3B", "#673770","#D14385", "#653936", "#C84348", 
  "#8569D5", "#5E738F","#D1A33D", "#8A7C64", "#599861", "black"
)
z <- ggplot(genusAN, aes(x = d, y = taxa.average, fill =Genus)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
