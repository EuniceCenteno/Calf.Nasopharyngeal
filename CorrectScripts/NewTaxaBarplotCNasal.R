...
#write.csv(Species, "BRDSpeciesHealthy.csv")
...
#write.csv(Species, "BRDSpeciesDraxxin.csv")
...
## Check the bacteria abundance of the animals that were healthy but got sick
#Calf 10 and 17
calf1017 = healthy[c(22:23,37:38),]
calf_OTU <- NewASVtable[rownames(NewASVtable) %in% rownames(calf1017),] #4, 3837

OTU.physeq = otu_table(as.matrix(calf_OTU), taxa_are_rows=FALSE)
tax.physeq = tax_table(as.matrix(NewTax))
#meta.physeq = sample_data(meta)
meta.physeq = sample_data(calf1017)
physeq = phyloseq(OTU.physeq, tax.physeq, meta.physeq)

##Making the tree, we need the phyloseq object 
random_tree = rtree(ntaxa(physeq), rooted=TRUE, tip.label=taxa_names(physeq))
##Merging the tree with the phyloseq object
physeqH1 = merge_phyloseq(physeq, meta.physeq, random_tree)
physeqH1 #This command should show the otu table, sample data, tax table and the phy tree
physeq_bar_plot = physeqH1
...
# Set colors for plotting
my_colors <- c(
  '#a6cee3','#b2df8a','orchid1',"wheat1",'#e31a1c',
  '#ff7f00','#cab2d6','seagreen3',"slategray2",'#6a3d9a','#b15928',"#CBD588"
)

my_colorsC1 <- c(
  '#1f78b4','palegreen2','#fb9a99',
  '#fdbf6f','#ff7f00',"#D1A33D",'#6a3d9a',"peachpuff1",'#b15928', 
  "#CBD588", "#5F7FC7", "orange"
)

my_colorsC2 <- c(
  '#a6cee3', "#C84248",'#1f78b4','#b2df8a','#33a02c','#fb9a99','#e31a1c',
  '#fdbf6f','#ff7f00','#cab2d6','#6a3d9a','#ffff99','#b15928', 
  "#CBD588", "#5F7FC7", "orange","#DA5724", "#508578", "#CD9BCD",
  "#AD6F3B", "#673770","#D14285", "#652926", "#C84248", 
  "#8569D5", "#5E738F","#D1A33D", "#8A7C64", "#599861", "gray", "black"
...
for(ml in my_level){
  ...
  ggplot(physeq_meta_filtered, aes(x = get(my_column), y = Abundance.average, fill = get(ml))) + 
    #facet_grid(.~LitterTreatment) +
    geom_bar(stat = "identity") +
    scale_fill_manual(values = my_colorsC2) +
    theme_bw()+
    # Remove x axis title
    #theme(axis.title.x = element_blank()) + 
    ylim(c(0,1)) +
    ...
    xlab(my_column) +
    ggtitle(paste0(ml, " (>", abund_filter * 100,"%) in at least 1 sample")) 
  ggsave(paste0("", ml, "BarPlot_Calf1017", my_column, ".png"), height = 5, width = 4)
}
write.csv(physeq_meta_filtered, "physeq_meta_filteredcalf1017Phylum.csv")

## Determine how the animals that remained healthy looked after removing calf 10 and 17
str(healthy)
hNo1017 = healthy[-c(22:23,37:38),]

calf_OTU2 <- NewASVtable[rownames(NewASVtable) %in% rownames(hNo1017),] #47, 3837

OTU.physeq2 = otu_table(as.matrix(calf_OTU2), taxa_are_rows=FALSE)
tax.physeq2 = tax_table(as.matrix(NewTax))
#meta.physeq = sample_data(meta)
meta.physeq2 = sample_data(hNo1017)
physeq2 = phyloseq(OTU.physeq2, tax.physeq2, meta.physeq2)

##Making the tree, we need the phyloseq object 
random_tree2 = rtree(ntaxa(physeq2), rooted=TRUE, tip.label=taxa_names(physeq2))
##Merging the tree with the phyloseq object
physeqH2 = merge_phyloseq(physeq2, meta.physeq2, random_tree2)
physeqH2 #This command should show the otu table, sample data, tax table and the phy tree
physeq_bar_plot2 = physeqH2


# Set colors for plotting
my_colors <- c(
  '#a6cee3','#1f78b4','#b2df8a','#33a02c','#e31a1c',
  '#fdbf6f','#ff7f00','#cab2d6',"olivedrab1",'#6a3d9a','#ffff99','#b15928', 
  "#CBD588", "#5F7FC7", "orange","#DA5724", "#508578", "#CD9BCD",
  "#AD6F3B", "#673770","#D14285", "#652926", "#C84248", 
  "#8569D5", "#5E738F","#D1A33D", "#8A7C64", "#599861", "gray", "black"
)
...
for(ml in my_level){
  ...
  ggplot(physeq_meta_filtered, aes(x = get(my_column), y = Abundance.average, fill = get(ml))) + 
    ...
    xlab(my_column) +
    ggtitle(paste0(ml, " (>", abund_filter * 100,"%) in at least 1 sample")) 
  ggsave(paste0("", ml, "BarPlot_healthy", my_column, ".png"), height = 5, width = 4)
}
write.csv(physeq_meta_filtered, "physeq_meta_filteredHealthyNo1017.csv")
...
#### plot new taxonomy for lactobacillus, you used GG
